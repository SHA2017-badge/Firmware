...
static bool badge_wifi_init_done = false;

static esp_err_t badge_wifi_event_handler(void *ctx, system_event_t *event) {
...
void badge_wifi_init(void) {
  ...
  badge_wifi_init_done = true;
}

void badge_wifi_wait(void) {
  if (badge_wifi_init_done) {
    xEventGroupWaitBits(wifi_event_group, CONNECTED_BIT, false, true,
                  portMAX_DELAY);
  } else {
    ESP_LOGI(TAG, "WiFi not initialized yet!");
  }
}
