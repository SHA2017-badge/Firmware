...
static voidFuncPtr __pinInterruptHandlers[GPIO_PIN_COUNT] = {
    0,
};
...
extern void IRAM_ATTR __pinMode(uint8_t pin, uint8_t mode) {
  ...
  if (mode & (INPUT | OUTPUT)) {
    ...
    pinFunction |=
        ((uint32_t)(((pin) == 1 || (pin) == 3) ? 0 : 1) << MCU_SEL_S);
  } else {
...
extern void __attachInterrupt(uint8_t pin, voidFuncPtr userFunc,
                              int intr_type) {
