...
void
gdeWriteCommand(uint8_t command)
{
...
void
gdeWriteCommandInit(uint8_t command)
{
	gpio_set_level(PIN_NUM_CS, HIGH);
	gpio_set_level(PIN_NUM_CS, LOW);
	gdeWriteByte(command);
...
gdeInit(void)
{
	...
	WRITE_PERI_REG(SPI_CLOCK_REG(SPI_NUM), (1 << SPI_CLKCNT_N_S) | (1 << SPI_CLKCNT_L_S)); // 40 MHz
