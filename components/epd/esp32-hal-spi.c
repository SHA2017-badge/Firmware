...
spi_t *spiStartBus(uint8_t spi_num, uint32_t clockDiv, uint8_t dataMode,
                   uint8_t bitOrder) {
  if (spi_num > 3) {
    return NULL;
  }
...
uint16_t spiReadWord(spi_t *spi) {
  ...
  while (spi->dev->cmd.usr)
    ;
  data = __spiTranslate16(spi->dev->data_buf[0] & 0xFFFF,
                          !spi->dev->ctrl.rd_bit_order);
  SPI_MUTEX_UNLOCK();
  return data;
}
...
void spiTransferBits(spi_t *spi, uint32_t data, uint32_t *out, uint8_t bits) {
  ...
  if (bytes == 1) {
    ...
    spi->dev->data_buf[0] =
        __spiTranslate16(data & mask, !spi->dev->ctrl.wr_bit_order);
  } else if (bytes == 3) {
    spi->dev->data_buf[0] =
        __spiTranslate24(data & mask, !spi->dev->ctrl.wr_bit_order);
  } else {
    spi->dev->data_buf[0] =
        __spiTranslate32(data & mask, !spi->dev->ctrl.wr_bit_order);
  }
  spi->dev->cmd.usr = 1;
  ...
  if (out) {
    ...
    if (bytes == 1) {
      ...
      *out = __spiTranslate16(spi->dev->data_buf[0] & mask,
                              !spi->dev->ctrl.wr_bit_order);
    } else if (bytes == 3) {
      *out = __spiTranslate24(spi->dev->data_buf[0] & mask,
                              !spi->dev->ctrl.wr_bit_order);
    } else {
      *out = __spiTranslate32(spi->dev->data_buf[0] & mask,
                              !spi->dev->ctrl.wr_bit_order);
    }
  }
  SPI_MUTEX_UNLOCK();
...
void __spiTransferBytes(spi_t *spi, uint8_t *data, uint8_t *out,
                        uint32_t bytes) {
  if (!spi) {
    return;
  }
